...
<!--
	...
	entity descriptor with an SP role, and produces a CSV file with the 
	following fields:
	
	  1. Organization Name: md:OrganizationName
	  2. SP Display Name: mdui:DisplayName
	  3. SP Entity ID: @entityID
...
<xsl:stylesheet version="1.0"
	...
	<xsl:template match="/">
	  <xsl:text>Organization Name,SP Display Name,SP Entity ID,Registrar ID</xsl:text>
	  <xsl:text>&#x0a;</xsl:text>
	  <xsl:apply-templates/>
	</xsl:template>
